...
def cal_class_auroc(nd1,and1,cls_list):
    # Class AUROC
...
def generate_prototypes(model, valid_loader, n_cluster=200, split = False):
    first = True
    with torch.no_grad():
        for idx, (pos_1, _, semi_target, _) in enumerate(valid_loader):
...
def test(model, test_loader, epoch):
    ...
    with torch.no_grad():
        ...
        for idx, (pos_1, _, _,  target) in enumerate(test_loader):
            ...
            cls_list.extend((1-target).tolist())
            distance.extend(Px_mean.tolist())
...
#                 and1.extend(Px_mean[negative_target].tolist())
    ...
    print("px\t", roc_auc_score(cls_list, distance))
    print()

#     cal_class_auroc(nd1, and1, cls_list)
    return
...
total_dataset = MC_Dataset('../OOD_data', args.ratio_known_normal, args.ratio_known_outlier, args.ratio_pollution, train_transform, test_transform, strong_aug)
...
rot_num = 4
validation_dataset = []
for i, (pos,pos2,semi_target, target) in tqdm(enumerate(valid_loader)):
    images1 = torch.cat([rotation(pos, k) for k in range(rot_num)])
    images2 = torch.cat([rotation(pos2, k) for k in range(rot_num)])
...
# test(model, test_loader, 0) # we do not test them
...
#         print("Decaying lr to {}".format(new_lr))
    ...
    losses_energy = []
    losses_shift = []
    for i, (pos, _,  semi_target, target) in tqdm(enumerate(train_loader)):
        pos = pos.to(device)
...
#         scheduler.step(epoch + i / len(train_loader))        
    ...
    # earlystop
    model.eval()
    test(model, test_loader, epoch) # we do not test them
    with torch.no_grad():
