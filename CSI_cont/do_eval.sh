...
for ratio_pollution in "${RATIO_POLLUTION_LIST[@]}"; do
    for one_class_idx in "${CLASS_LIST[@]}"; do
        CMD=${MAIN_CMD}" --one_class_idx ${one_class_idx} --load_path ${MODEL_DIR}/cifar10_resnet18_unsup_simclr_CSI_shift_rotation_one_class_${one_class_idx}_ratio_pollution_${ratio_pollution}/last.model"
        echo ${CMD}
        ${CMD} | tee ${SAVE_DIR}/csi_lars_one_class_${one_class_idx}_ratio_pollution_${ratio_pollution}
    done
done
